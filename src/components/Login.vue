...
<script>
  export default {
    ...
    methods: {
      resetLoginForm () {
        console.log(this)
        this.$refs.loginFormRef.resetFields()// 重置表单为初始状态
      },
      login () {
        this.$refs.loginFormRef.validate(async valid => {
          ...
          window.sessionStorage.setItem('token', res.data.token) // 登录成功后将token保存到sessionStorage中，以便设置登录拦截
          this.$router.push('/home') // 跳转到home组件页面
        })
      }
    }
  }
